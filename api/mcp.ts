  ...
  paymentMiddleware(
    {
      "POST /api/mcp": {
        ...
        extensions: {
          // Replace with real logic once deployed
          "bc_1aw46v36": declareBuilderCodeExtension("bc_1aw46v36"),
        },
      },
    },
    resourceServer
...
app.post("/api/mcp", (req, res) => {
  ...
  try {
    ...
    if (method === "tools/call" || action === "tools/call") {
      ...
      switch (toolName) {
        ...
        case "get_leaderboard":
          toolText = JSON.stringify({
            ...
            contract: { network: "Base Mainnet", chainId: 8453, resolverAddress: "0x019a3bc4d22da10ac73d8e53415d37aa96045330a" },
            timestamp: new Date().toISOString()
          }, null, 2);
          break;
      ...
      result = { content: [{ type: "text", text: toolText }], isError: isError };
      const responsePayload = isJsonRpc ? { jsonrpc: "2.0", id, result } : result;
      return res.status(200).json(responsePayload);
    }
    ...
    switch (action || command) {
      case "status":
      case "ping":
        result = { status: "online", agent: "Endless Runner Orchestrator", message: "Ready to run endlessly" };
        break;

      case "execute":
        result = { success: true, action: command || params, executedAt: new Date().toISOString(), message: "Command executed in Endless Runner" };
        break;

      case "get_info":
        result = { name: "Endless Runner Orchestrator", wallet: "0xe157F1F5e12adB38Ba013683E9Ce24efe21e5bA6", platform: "Base", version: "1.0.0" };
        break;
    ...
    return res.status(200).json({ status: "success", agent: "Endless Runner Orchestrator", response: result, receivedAt: new Date().toISOString() });
  } catch (error) {
